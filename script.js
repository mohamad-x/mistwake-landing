...
(function improveMistWakePage() {
      ...
      <div class="product-gif-card">
        ...
        <div class="product-gif-copy">
          ...
          <div class="gif-proof-points" aria-label="MistWake demo highlights">
            <span>Controlled mist cue</span>
            <span>Adjustable nozzle</span>
            <span>Sound-only mode</span>
          </div>
  ...
  function addKickstarterSpecificFaqs() {
    ...
    projectFaq.innerHTML = `<h3>Is this a finished retail product?</h3><p>Not yet. MistWake is preparing for Kickstarter. The working prototype proves the core function, and the final version may still be refined before manufacturing.</p>`;
  ...
  function refineCopy() {
    ...
    if (founderText) {
      founderText.textContent = "I built MistWake because sound-only alarms failed me when waking up mattered. The goal is simple: sound first, then a controlled physical cue when sound is not enough.";
    }

    const roadmapHeader = document.querySelector(".roadmap-section .section-header p:last-child");
    if (roadmapHeader) {
      roadmapHeader.textContent = "Here is the current path from working prototype to Kickstarter launch, manufacturing refinement, production, and fulfillment.";
    }
    ...
    const funnelCopy = [
      ["1. Reserve your spot", "Secure VIP launch access before the Kickstarter campaign opens."],
      ["2. Get notified first", "VIP supporters will be notified when the campaign goes live."],
      ["3. Decide on Kickstarter", "The $1 reservation does not buy the product. You will still choose whether to pledge on Kickstarter."]
    ];
